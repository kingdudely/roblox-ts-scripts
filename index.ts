...
const u8 = Uint8Array, u16 = Uint16Array, i16 = Int16Array, i32 = Int32Array;
...
const rzfh = (dat: Uint8Array, w?: Uint8Array | 1): number | DZstdState => {
  const n3 = buffer.readbits(dat.buffer, 0, 24); // dat[0] | (dat[1] << 8) | (dat[2] << 16);
  if (tonumber(n3) === 0x2FB528 && tonumber(dat[3]) === 253) {
    ...
    // dictionary id
    const di = buffer.readbits(dat.buffer, bt * 8, db * 8);
    bt += db;
    // frame size bytes
    const fsb = fcf ? (1 << fcf) : ss;
    // frame source size
    const fss = buffer.readbits(dat.buffer, bt * 8, fsb * 8) + (((tonumber(fcf) === 1) && 256) ? 1 : 0);
    // window size
    let ws = fss;
    if (!ss) {
      // window descriptor
    ...
    // skippable
    return buffer.readu32(dat.buffer, 4) + 8; // b4(dat, 4) + 8;
  }
  throw ec[0];
};
...
const rhu = (dat: Uint8Array, bt: number): [number, HDT] => {
  ...
  if (tonumber(ri[0]) !== tonumber(ts)) throw ec[0];
  for (i = 0; i < wc; ++i) {
    const bits = hw[i];
    if (bits) {
      const code = ri[bits];
...
const b2bl = (b: Uint8Array, s: number) => {
  const len = getn(b), bl = new i32(len);
  for (let i = 0; i < len; ++i) {
    bl[i] = s;
    s += 1 << b[i];
  }
...
const dhu = (dat: Uint8Array, out: Uint8Array, hu: HDT) => {
  const len = getn(dat), ss = getn(out), lb = dat[len - 1], msk = (1 << hu.b) - 1, eb = -hu.b;
  if (!lb) throw ec[0];
  ...
  if (tonumber(pos) !== tonumber(eb) || tonumber(i + 1) !== tonumber(ss)) throw ec[0];
};
...
const dhu4 = (dat: Uint8Array, out: Uint8Array, hu: HDT) => {
  let bt = 6;
  const ss = getn(out), sz1 = (ss + 3) >> 2, sz2 = sz1 << 1, sz3 = sz1 + sz2;
